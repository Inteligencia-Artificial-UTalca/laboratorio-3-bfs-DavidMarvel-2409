    ...
    float Search::Heuristic(std::pair<int,int> start, std::pair<int,int> goal){
        ...
        return (valFirst + valSecond) + (1.41f - 2.0f) * std::min(valFirst,valSecond);
    }
    ...
    std::vector<std::pair<int,int>> Search::Astar(const Map & map, std::pair<int,int> start, std::pair<int,int> goal) {
        ...
        while(!OPEN.empty()){
            ...
            for(auto dir:dirs){
                ...
                if (dx != 0 && dy != 0) {
                    if (!map.isWalkable(pos.first + dx, pos.second) ||
                        !map.isWalkable(pos.first, pos.second + dy)) {
                        continue;
                    }
                }

                float cost = (dx != 0 && dy != 0) ? 1.41f : 1.0f;
    ...
    std::vector<std::pair<int,int>> Search::WeightedAstar(const Map & map, std::pair<int,int> start, std::pair<int,int> goal, float peso) {
        std::cout<<"===========================\nRunning Weighted AStar...\n";
        auto startTime = std::chrono::high_resolution_clock::now();
        ...
        auto comparaHeuristica = [&](std::pair<int,int> a, std::pair<int,int> b){
            float fA = gScore[a.first][a.second] + peso * Heuristic(a,goal);
            float fB = gScore[b.first][b.second] + peso * Heuristic(b,goal);
            return fA > fB;
        };
        //std::queue<std::pair<int,int>> OPEN;
        std::priority_queue<std::pair<int,int>, std::vector<std::pair<int,int>>, decltype(comparaHeuristica)> OPEN(comparaHeuristica);
